...
[bold color(141)]
         ...
         ╚═╝   ╚═╝
[/bold color(141)]
[dim]                             D E E P   R E S E A R C H[/dim]
"""


def print_banner(config_name: str = ""):
    """Print the Parallax DR banner."""
    if HAS_RICH:
        console.print(PARALLAX_BANNER)
        if config_name:
            mode = "Deep" if "deep" in config_name.lower() else "Quick"
...
async def chat_loop(
    ...
    def format_citations(t):
        ...
        t = re.sub(
            r'<cite\s+ids?=(["\']?)([^"\'>\s]+)\1[^>]*>([^<]+)</cite>', format_cite, t
        )
        ...
        if text and len(text) < last_processed_text_len:
            ...
            if active_live:
                ...
                if is_answering:
                    active_live.update(
                        render_panel(
                            current_segment_text, "Answer", "green", is_active=False
                        )
                    )
                else:
                    active_live.update(
                        render_panel(
                            current_segment_text, "Thinking", "yellow", is_active=False
                        )
                    )
                active_live.stop()
                active_live = None
                ...
                if answer_match:
                    ...
                    if not is_answering:
                        ...
                        if active_live:
                            ...
                            active_live.update(
                                render_panel(
                                    thinking_text, "Thinking", "yellow", is_active=False
                                )
                            )
                            active_live.stop()
                        ...
                        current_segment_text = (
                            answer_text  # Update with latest answer text
                        )
                        active_live.update(
                            render_panel(
                                current_segment_text,
    ...
    while True:
        try:
            ...
            if active_live:
                if is_answering:
                    final_answer_text = (
                        current_segment_text  # Store final answer for bibliography
                    )
                else:
                    thinking_text = (
                        current_segment_text  # Store thinking text for display
                    )
            ...
            if active_live:
                if is_answering:
                    active_live.update(
                        render_panel(
                            format_citations(final_answer_text),
                            "Answer",
                            "green",
                            is_active=False,
                        )
            ...
            if verbose and searched_links:
                ...
                if len(searched_links) > 5:
                    console.print(
                        f"[dim]  ... and {len(searched_links) - 5} more[/dim]"
                    )
...
def chat(
    ...
    # Print the Parallax banner
    print_banner(config)

    # Create workflow
    try:
        workflow = AutoReasonSearchWorkflow(configuration=config, **overrides)
...
if __name__ == "__main__":
    if HAS_RICH:
        ...
        parser = argparse.ArgumentParser(
            description="Interactive chat using auto_search workflow"
        )
        parser.add_argument(
            "--config",
            "-c",
