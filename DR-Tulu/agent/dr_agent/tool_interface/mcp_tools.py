...
RAG_MCP_PATH = Path(dr_agent.__file__).parent / "mcp_backend" / "run_stdio.py"
...
# Limits to reduce overload downstream
MAX_SNIPPET_LEN = 500
MAX_DOCS_DEFAULT = 5


def smart_clean_content(url: str, content: str, max_chars: int = 1200) -> str:
    """
    Simple cleaner to drop boilerplate and trim oversized content. Works on plain text
    returned by fetchers (e.g., Jina) using domain-specific heuristics plus generic filters.
    """
    if not content:
        return ""
    domain = ""
    ...
    # Domain-specific light selectors on plain text
    lower = text.lower()
    if "arxiv.org" in domain:
        m = re.search(r"abstract[:\s]*", lower)
        if m:
            start = m.end()
            stop_match = re.search(r"(comments|subjects|\n\n)", lower[start:])
            end = start + stop_match.start() if stop_match else None
            text = text[start:end].strip() or text
    elif "huggingface.co" in domain:
        lines = [
            ln for ln in text.splitlines()
            if ln.strip() and not ln.lower().startswith(("submitted by", "uploaded by"))
        ]
        text = "\n".join(lines)
        ...
        for ln in lines:
            ll = ln.lower()
            if any(k in ll for k in ["license", "contributors", "sponsor", "footnote"]):
                continue
            trimmed.append(ln)
        text = "\n".join(trimmed)
...
class MCPMixin:
    ...
    def init_mcp_client(self):
        ...
        if transport_type == "StreamableHttpTransport":
            ...
            # Prefer in-memory transport to avoid subprocess/stdio noise.
            try:
                from dr_agent.mcp_backend import main as mcp_module

                logger.debug("Using in-memory FastMCP transport")
                transport = FastMCPTransport(mcp_module.mcp)
                return Client(transport, timeout=self.timeout)
            except Exception as e:
                logger.warning(
                    f"Falling back to stdio MCP transport due to import error: {e}"
                )
                ...
                logger.debug(
                    f"Using MCP transport via stdio executable: {self.mcp_executable}"
                )
                transport = PythonStdioTransport(
                    script_path=self.mcp_executable,
    ...
    def extract_documents(self, raw_output: Dict[str, Any]) -> List[Document]:
        ...
        for result in organic_results:
            if isinstance(result, dict):
                ...
                snippet = smart_clean_content(url, result.get("snippet", "").strip(), max_chars=MAX_SNIPPET_LEN)
                key = (title, url)
                if key in seen:
                    continue
                seen.add(key)
                doc = Document(title=title, url=url, snippet=snippet, text=None, score=None)
                if doc.title or doc.snippet or doc.url:
                    documents.append(doc)
                if len(documents) >= MAX_DOCS_DEFAULT:
                    break

        return documents


class ExaSearchTool(MCPSearchTool):
    """Tool for web search using Exa API via MCP"""

    def __init__(
        self,
        tool_parser: Optional[ToolCallParser | str] = None,
        number_documents_to_search: int = 5,
        timeout: int = 60,
    ...
    def get_mcp_params(self, tool_call_info: ToolCallInfo) -> Dict[str, Any]:
        ...
        # Allow overrides from tool call
        if "num_results" in tool_call_info.parameters:
            try:
                params["num_results"] = int(tool_call_info.parameters["num_results"])
            except (TypeError, ValueError):
                pass
        if "search_type" in tool_call_info.parameters:
            params["search_type"] = str(tool_call_info.parameters["search_type"])
        if "use_autoprompt" in tool_call_info.parameters:
            try:
                params["use_autoprompt"] = bool(tool_call_info.parameters["use_autoprompt"])
            except Exception:
                pass
        ...
        # Tool call overrides for domain filters (comma-separated)
        if "include_domains" in tool_call_info.parameters:
            raw = tool_call_info.parameters["include_domains"]
            if isinstance(raw, str):
                include_domains = [d.strip() for d in raw.split(",") if d.strip()]
        if "exclude_domains" in tool_call_info.parameters:
            raw = tool_call_info.parameters["exclude_domains"]
            if isinstance(raw, str):
                exclude_domains = [d.strip() for d in raw.split(",") if d.strip()]
    ...
    def extract_documents(self, raw_output: Dict[str, Any]) -> List[Document]:
        results = raw_output.get("results") or raw_output.get("data") or []
        ...
        for item in results:
            ...
            snippet = smart_clean_content(url, snippet, max_chars=MAX_SNIPPET_LEN)
            key = (title, url)
            if key in seen:
                continue
            seen.add(key)
            ...
            doc = Document(
                title=title or url or "Exa result",
                url=url,
                snippet=snippet,
                text=None,
                score=item.get("score"),
...
class ExaWebsetsSearchTool(ExaSearchTool):
    """Tool for searching curated Exa Websets collections via MCP"""
    ...
    def get_mcp_params(self, tool_call_info: ToolCallInfo) -> Dict[str, Any]:
        ...
        # Use webset_id from config unless overridden in call
        webset_id = self.webset_id
        if "webset_id" in tool_call_info.parameters:
            webset_id = str(tool_call_info.parameters["webset_id"])
        if webset_id:
            params["webset_id"] = webset_id
        return params

class ArabicBooksSearchTool(MCPSearchTool):
    """Tool for searching private Arabic classical texts via MCP-backed ChromaDB"""
    ...
    def get_mcp_tool_name(self) -> str:
        return "arabic_books_search"

    def get_mcp_params(self, tool_call_info: ToolCallInfo) -> Dict[str, Any]:
        """Build parameters for the Chroma-backed Arabic library search"""
        params: Dict[str, Any] = {
            "query": tool_call_info.content,
            "n_results": self.number_documents_to_search,
        }
        if "n_results" in tool_call_info.parameters:
            try:
                params["n_results"] = int(tool_call_info.parameters["n_results"])
            except (TypeError, ValueError):
                pass
        if self.chroma_path:
            params["chroma_path"] = self.chroma_path
        if self.collection_name:
            params["collection_name"] = self.collection_name
        return params

    def extract_documents(self, raw_output: Dict[str, Any]) -> List[Document]:
        """Extract passages plus citation metadata from Arabic library search"""
        records = raw_output.get("results", []) or []
        documents: List[Document] = []
        ...
        for item in records:
            ...
            title = (metadata.get("book_title") or metadata.get("title") or "").strip()
            volume = metadata.get("volume") or metadata.get("vol")
            ...
            if volume is not None:
                citation_parts.append(f"m. {volume}")
            if page_start is not None:
                if page_end:
                    citation_parts.append(f"pp. {page_start}-{page_end}")
                else:
            ...
            snippet_lines = [text]
            if citation:
                snippet_lines.append(f"Source: {citation}")

            doc = Document(
                title=title or "Arabic library passage",
                url="",
                snippet="\n".join(snippet_lines),
                text=None,
...
class MCPBrowseTool(MCPMixin, BaseTool, ABC):
    ...
    async def _fetch_webpages_parallel(
        ...
        # Dedupe by URL/title to avoid repeated fetches
        seen_pairs = set()
        docs_with_urls = []
        for doc in documents:
            key = (doc.url, doc.title)
            if not doc.url or key in seen_pairs:
                continue
            seen_pairs.add(key)
    ...
    async def __call__(
        ...
        for doc_id, raw_output in fetch_results.items():
            ...
            if error := raw_output.get("error"):
                ...
                # Clean and trim the raw content to reduce boilerplate
                document.text = smart_clean_content(
                    document.url, raw_content or document.snippet or ""
                )
