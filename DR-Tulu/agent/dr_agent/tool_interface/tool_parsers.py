...
class UnifiedToolCallParserV20250824(ToolCallParser):
    ...
    def has_calls(self, text: str, tool_name: str) -> bool:
        """Check if this parser can find calls for the given tool in the text"""
        # Look for <call_tool name="toolname"> patterns matching this tool
        # Also check for incomplete calls (API might stop at stop sequence)
        for pattern in [
    ...
    def parse_call(self, text: str, tool_name: str) -> Optional[ToolCallInfo]:
        """Parse the first tool call for the given tool in the text, including parameters and position"""
        # Find <call_tool name="toolname" ...>content</call_tool> with position info
        # Also handle incomplete tool calls where closing tag is missing (API stopped at stop sequence)
        for pattern in [
            r"<call_tool\s+([^>]*?)>(.*?)</call_tool>",
            r"<call_tool\s+([^>]*?)>(.*?)</call>",
            r"<call_tool\s+([^>]*?)>([^<]*?)$",  # Handle incomplete (no closing tag at end of text)
        ]:
            for match in re.finditer(pattern, text, re.DOTALL):
                attr_string = match.group(1)
                content = match.group(2).strip()
...
@register_parser("qwen_tool_call")
class QwenToolCallParser(ToolCallParser):
    """
    Parser for Qwen's <tool_call>{"name": "xxx", "arguments": {...}}</tool_call> format.

    This format is used by Qwen models when they receive OpenAI-style tools parameter.
    The model outputs JSON inside <tool_call> tags instead of using the tool_calls field.
    """
    ...
    @property
    def stop_sequences(self) -> List[str]:
        """Get stop sequences for Qwen tool_call format"""
        return ["</tool_call>"]
    ...
    def has_calls(self, text: str, tool_name: str) -> bool:
        ...
        for match in re.finditer(pattern, text, re.DOTALL):
            try:
                ...
                if call_data.get("name") == tool_name:
                    return True
        ...
        for match in re.finditer(pattern, text, re.DOTALL):
            try:
                ...
                if call_data.get("name") == tool_name:
                    ...
                    # For content, use the first string argument or empty string
                    content = ""
                    for key in ["query", "url", "input", "text", "content"]:
                        if key in arguments:
                            content = str(arguments[key])
                            break
    ...
    def format_result(self, formatted_output: str, output: "ToolOutput") -> str:
        ...
        return f"<tool_response>{formatted_output}</tool_response>"
    ...
    def format_tool_call(
        ...
        """Format a tool call in Qwen's format"""
        import json

        call_data = {"name": tool_name, "arguments": arguments}
