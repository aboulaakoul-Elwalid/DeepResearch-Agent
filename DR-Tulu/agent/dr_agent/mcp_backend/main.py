...
@mcp.tool(tags={"search"})
def exa_search(
    query: Annotated[str, "Search query for Exa"],
    num_results: Annotated[int, "Number of results to return (default 5, max 20)"] = 5,
    include_domains: Annotated[
        Optional[str],
        "Comma-separated domains to include (e.g., 'example.com,arxiv.org')",
    ] = None,
    ...
    Requires EXA_API_KEY in the environment. Returns the raw Exa response
    (results with title/url/text/score) plus request metadata.
    """
    try:
        include_list = (
        ...
        return {"error": f"Unexpected Exa error: {exc}", "data": []}


@mcp.tool(tags={"search"})
def exa_websets_search(
    query: Annotated[str, "Search query for Exa Websets"],
    webset_id: Annotated[
        Optional[str], "Optional Webset ID to constrain results"
    ] = None,
    num_results: Annotated[int, "Number of results to return (default 5, max 20)"] = 5,
    include_domains: Annotated[
        Optional[str],
    ...
    """
    Search Exa Websets collections. Requires EXA_API_KEY.
    """
    try:
        include_list = (
            [d.strip() for d in include_domains.split(",") if d.strip()]
        ...
        return {"error": f"Unexpected Exa Websets error: {exc}", "data": []}


@mcp.tool(tags={"search", "necessary"})
def arabic_books_search(
    query: Annotated[str, "Search query in Arabic or English"],
    n_results: Annotated[int, "Number of passages to return (default 5, max 10)"] = 5,
    chroma_path: Annotated[
        Optional[str],
        "Optional override for the Chroma persistence path (defaults to ARABIC_BOOKS_CHROMA_PATH env or /home/elwalid/projects/parallax_project/chroma_db)",
    ] = None,
    collection_name: Annotated[
        Optional[str],
        "Optional override for the Chroma collection name (defaults to ARABIC_BOOKS_COLLECTION env or 'arabic_books')",
    ] = None,
) -> dict:
    """
    Search a private ChromaDB collection of classical Arabic texts (tafsir, hadith, fiqh, history).
