...
export interface ClusterActions {
  ...
  /**
   * Refresh cluster connections after the API base URL changes.
   */
  readonly refreshApiBase: (nextBaseUrl?: string) => Promise<void>;
}
...
export const ClusterProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const streamRef = useRef<ReturnType<typeof createStreamClusterStatus> | null>(null);

  const rebuildStream = useRefCallback(() => {
  ...
  const refreshApiBase = useRefCallback<ClusterActions['refreshApiBase']>(async (nextBaseUrl) => {
    if (nextBaseUrl) {
